...
Most of them have default values, that can be overriden by exposing a different value as environment variable.
...
Before each commit passes automatic tests and linting is run. If any action failes, the commit failes.
