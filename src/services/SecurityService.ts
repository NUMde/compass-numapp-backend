    ...
    public static decryptLogin(ciphertext: string, encryptedKey: string, nonce: string): string {
        try {
            ...
            try {
                ...
                Logger.Err(
                    '[SecurityService.decryptLogin][key_description_failed] ' + JSON.stringify(err)
                );
            ...
            if (err !== 'key_description_failed') {
                Logger.Err(
                    '[SecurityService.decryptLogin][description_failed] ' + JSON.stringify(err)
                );
            }
